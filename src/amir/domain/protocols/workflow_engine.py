...
class WorkflowEngine(ABC):
    ...
    @abstractmethod
    async def create_workflow(self, template_id: UUID, team_id: UUID) -> UUID:
        """Create workflow instance from template.
        Returns workflow_instance_id.
        """

    @abstractmethod
    async def next_state(self, workflow_id: UUID, completed_task_id: UUID, task_status: str) -> str | None:
        """Determine next workflow state.
        Returns next state name or None if complete.
        """

    @abstractmethod
    async def mark_state(self, workflow_id: UUID, state: str) -> None:
        """Update workflow to specific state."""

    @abstractmethod
    async def get_state(self, workflow_id: UUID) -> str:
