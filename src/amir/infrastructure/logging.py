...
def _format_utc_timestamp(a_epoch_seconds: float) -> str:
    ...
    dt = datetime.fromtimestamp(a_epoch_seconds, tz=UTC)
...
def _build_logging_config(a_config: LoggingConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "handlers": handlers,
        "loggers": {
            name: {"level": a_config.level, "handlers": [], "propagate": True} for name in _PROPAGATING_LOGGERS
        },
...
class StructuredLogSink:
    ...
    def __init__(self, a_logger_name: str = APPLICATION_LOGGER_NAME) -> None:
        self._logger = logging.getLogger(a_logger_name)

    def write(self, a_level: str, a_message: str, a_resource: str = "") -> None:
        log_level = getattr(logging, a_level.upper(), logging.INFO)
        extra: dict[str, str] = {}
        if a_resource:
            extra["resource"] = a_resource
        self._logger.log(log_level, a_message, extra=extra)
